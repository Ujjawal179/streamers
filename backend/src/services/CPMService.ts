...
export class CPMService {
  static getCPMTier(ccv: number): CPMTier {
    ...
    if (!tier) {
      throw new ApiError(400, 'Invalid CCV value');
    }
    return tier;
  }

  static calculateStreamIncome(ccv: number): {
    cpmRate: number;
    maxAdsPerStream: number;
    maxIncomePerStream8Ads: number;
    maxIncomePerStream16Ads: number;
  ...
  static calculateCampaignMetrics(ccv: number, budget: number): {
    cpmRate: number;
    maxAdsPerStream: number;
    possiblePlays: number;
  ...
  static validateAdScheduling(ccv: number, requestedPlays: number): {
    isValid: boolean;
    message: string;
    recommendedDays?: number;
  ...
  static calculateYoutuberEarnings(ccv: number, plays: number): {
    totalAmount: number;
    platformFee: number;
    youtuberEarnings: number;
    estimatedViews: number;
    cpmRate: number;
    costPerPlay: number;
  } {
  ...
  static getStreamingConstraints(ccv: number): {
    maxAdsPerStream: number;
    minInterval: number;
    maxAdDuration: number;
    totalMaxAdTime: number;
